			...
			throws IllegalStateException, NullPointerException
	{
		...
		if(packetData.readableBytes() > COMMON_MAX_CUSTOM_PAYLOAD_SIZE)
		{
			TCDCommonsNetwork.sendFracturedCpnPacket(
					...
					fp -> this.player.networkHandler.sendPacket(new CustomPayloadC2SPacket(
							fp.getPacketId(),
		...
		else this.player.networkHandler.sendPacket(new CustomPayloadC2SPacket(packetId, new PacketByteBuf(packetData)));
