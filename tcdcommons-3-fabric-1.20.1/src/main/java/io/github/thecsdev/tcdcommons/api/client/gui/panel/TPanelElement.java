...
public @Virtual class TPanelElement extends TElement
{
	...
	{
		//input vertically when has vertical or both flags
		if(hasScrollFlag(SCROLL_VERTICAL) || hasScrollFlag(SCROLL_BOTH))
			this.scrollVelocityY += scrollAmount;
		//input horizontally when has only horizontal flag
		else if(hasScrollFlag(SCROLL_HORIZONTAL))
			this.scrollVelocityX += scrollAmount;
		//and do nothing if has no flags
		else return false;
