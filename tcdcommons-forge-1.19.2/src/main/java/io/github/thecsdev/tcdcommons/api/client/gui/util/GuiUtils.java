...
public final class GuiUtils
{
	...
	public static void enableScissor(Minecraft client, int x, int y, int width, int height)
	{
		...
		x = (int) (x * scale);
		y = (int) (client.getWindow().getScreenHeight() - (y + height) * scale);
	...
	{
		return color | Mth.ceil(alpha * ((float)(color & 0xFF) / 255)) << 24;
	}
	// ==================================================
	/**
	 ...
	 * <br/>
	 * {@link MinecraftClient#screen} is passed at the parent screen.<br/>
	...
	{
		return showUrlPrompt(Minecraft.getInstance().screen, url, trusted);
	}
	
	/**
	 * Shows the user a prompt screen to open a given web URL.
	 * @param parent The currently opened screen, aka the screen that
	 * will be opened once the {@link ConfirmChatLinkScreen} is closed.
	 * @param url The web URL to open.
	 * @param trusted Whether or not the given URL can be trusted.
	 */
