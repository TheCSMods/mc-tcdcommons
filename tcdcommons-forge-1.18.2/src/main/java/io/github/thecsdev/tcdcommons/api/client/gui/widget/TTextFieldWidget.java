...
public class TTextFieldWidget extends TElement
{
	...
	{
		String txt = getText();
		Component message = TextUtils.literal(txt);
		int w = Math.max(getTpeWidth() - (TEXT_PADDING * 2), 10);
		this.multilineText = MultiLineLabel.create(getTextRenderer(), message, w);
		//invoke event
		if(invokeEvent)
	...
	{
		int fh = getTextRenderer().lineHeight + getLineSpacing();
		if(this.multilineText == null) return fh;
		return this.multilineText.getLineCount() * fh;
	}
	// --------------------------------------------------
	/**
