...
public abstract class TScreen extends Screen implements TParentElement
{
	 ...
	 */
	public ItemRenderer getItemRenderer() { return getClient().getItemRenderer(); }
	// --------------------------------------------------
			...
			TElement child, boolean isPreRender)
	{
		...
		{
			if(isPreRender) child.render(pencil, mouseX, mouseY, delta);
			else child.postRender(pencil, mouseX, mouseY, delta);
		});
