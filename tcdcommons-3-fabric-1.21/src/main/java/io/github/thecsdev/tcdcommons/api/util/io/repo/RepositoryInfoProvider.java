...
public abstract class RepositoryInfoProvider
{
	...
	static
	{
		...
		REPO_INFO_PROVIDER.register(Identifier.of(getModID(), "github"), GitHubRepositoryInfoProvider.getInstance());
	}
	// ==================================================
			...
			final Callable<T> infoObtainer) throws NullPointerException
	{
		...
		THREAD_SCHEDULER.submit(() ->
		{
			...
			catch(Error err) { minecraftClientOrServer.executeSync(() -> { throw err; }); return; }
	 ...
	 * @apiNote The {@link Consumer}s are invoked on the {@link ThreadExecutor}'s main {@link Thread}.
