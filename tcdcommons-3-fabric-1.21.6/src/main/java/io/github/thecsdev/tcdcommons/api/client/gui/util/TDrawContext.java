...
public final class TDrawContext extends DrawContext
{
	...
	public final void drawTElementTextTHSCS(Text text, HorizontalAlignment horizontalAlgnment, int sideOffset, int color, float textScale)
	{
		...
		getMatrices().translate((float)textX, (float)textY);
		getMatrices().scale(textScale, textScale);
		drawText(txtR, text, 0, 0, color, true);
			...
			int s)
	{
		...
		{
			//the four corners
			drawTexture(RenderPipelines.GUI_TEXTURED, textureId, x, y, s, s, u, v, s, s, tW, tH);
			drawTexture(RenderPipelines.GUI_TEXTURED, textureId, x + w - s, y, s, s, u + uvW - s, v, s, s, tW, tH);
			drawTexture(RenderPipelines.GUI_TEXTURED, textureId, x, y + h - s, s, s, u, v + uvH - s, s, s, tW, tH);
			drawTexture(RenderPipelines.GUI_TEXTURED, textureId, x + w - s, y + h - s, s, s, u + uvW - s, v + uvH - s, s, s, tW, tH);
			
			//the four sides
			drawTexture(RenderPipelines.GUI_TEXTURED, textureId, x + s, y, w - s2, s, u + s, v, uvW - s2, s, tW, tH);
			drawTexture(RenderPipelines.GUI_TEXTURED, textureId, x, y + s, s, h - s2, u, v + s, s, uvH - s2, tW, tH);	
			drawTexture(RenderPipelines.GUI_TEXTURED, textureId, x + w - s, y + s, s, h - s2, u + uvW - s, v + s, s, uvH - s2, tW, tH);
			drawTexture(RenderPipelines.GUI_TEXTURED, textureId, x + s, y + h - s, w - s2, s, u + s, v + uvH - s, uvW - s2, s, tW, tH);
		...
		else
		{
			...
			drawTexture(RenderPipelines.GUI_TEXTURED, textureId, x, y, w, h, u, v, uvW, uvH, tW, tH);
		}
	}
	// --------------------------------------------------
	/**
			...
			int textureWidth, int textureHeight)
	{
		...
		for(int x1 = x; x1 < endX; x1 += uvRegionWidth)
		{
			...
			drawTexture(RenderPipelines.GUI_TEXTURED, textureId, x1, y1, nextW, nextH, u, v, nextW, nextH, textureWidth, textureHeight);
		}
	}
	// --------------------------------------------------
	/**
	...
	public final void drawTButton(boolean enabled, boolean focused)
	{
		...
		drawGuiTexture(RenderPipelines.GUI_TEXTURED, BUTTON_TEXTURES.get(enabled, focused),
				curr.getX(), curr.getY(),
	...
	public final void drawTCheckbox(int x, int y, int width, int height, boolean highlighted, boolean checked)
	{
		...
		drawGuiTexture(RenderPipelines.GUI_TEXTURED, tex, x, y, width, height);
	}
	// ==================================================
	@Experimental
