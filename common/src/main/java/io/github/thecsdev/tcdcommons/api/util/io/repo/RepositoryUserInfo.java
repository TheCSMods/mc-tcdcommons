...
public abstract class RepositoryUserInfo
{
	 ...
	 * @throws ResourceLocationException If {@link RepositoryHostInfo#getID()}
	 * returns a {@link String} that is incompatible with {@link ResourceLocation} namespaces.
	 * @throws IOException If an {@link IOException} is raised while fetching.
	 * @see #getAvatarImageURI()
	 */
	public @Virtual byte[] fetchAvatarImageSync() throws NullPointerException, ResourceLocationException, IOException
	{
		//prepare to fetch; obtain needed variables
		final URI imgUri = Objects.requireNonNull(getAvatarImageURI());
		final ResourceLocation imgId = ResourceLocation.fromNamespaceAndPath(
		...
		CachedResourceManager.getResourceSync(imgId, new IResourceFetchTask<byte[]>()
		{
			...
			public BlockableEventLoop<?> getMinecraftClientOrServer() { return null; }
