...
public abstract class TAbstractMappedRegistry<T> implements TRegistry<T>
{
	...
	// ==================================================
	public final @Override Iterator<Entry<ResourceLocation, T>> iterator() { return this.map.entrySet().iterator(); }
	// --------------------------------------------------
	public final int size() { return this.map.size(); } //override for better performance
	// ==================================================
	public @Virtual @Override T register(ResourceLocation id, T entry)
	...
	// --------------------------------------------------
	public @Virtual @Override T unregister(ResourceLocation id) throws UnsupportedOperationException, NullPointerException
	{
	...
	// ==================================================
	public final @Override Optional<ResourceLocation> getKey(T entry)
	{
	...
	// --------------------------------------------------
	public final @Override Optional<T> getValue(ResourceLocation id)
	{
		return Optional.ofNullable(this.map.getOrDefault(id, null));
	}
	// ==================================================
	public final @Override boolean containsKey(ResourceLocation id) { return this.map.containsKey(id); }
	public final @Override boolean containsValue(T entry) { return this.map.containsValue(entry); }
	// ==================================================
	public final @Override void forEach(Consumer<? super Entry<ResourceLocation, T>> action) { TRegistry.super.forEach(action); }
	public final @Override Spliterator<Entry<ResourceLocation, T>> spliterator() { return TRegistry.super.spliterator(); }
	// ==================================================
	/**
	 * @see MappedRegistryEvent#invoke(ResourceLocation, Object)
	 */
	public static interface MappedRegistryEvent<T>
	{
		 ...
		 * 
		 * @param entryId The {@link ResourceLocation} of the entry being (un)registered.
		 * @param entry The entry being (un)registered.
		 * 
		 * @see TRegistry#register(ResourceLocation, Object)
		 * @see TRegistry#unregister(ResourceLocation)
		 * 
		 * @apiNote For {@link TAbstractMappedRegistry#eUnRegistered}, the entry
		 * {@link ResourceLocation} and/or the entry itself may be {@code null}, beware.
		 */
		public void invoke(ResourceLocation entryId, T entry);
	}
	// ==================================================
}