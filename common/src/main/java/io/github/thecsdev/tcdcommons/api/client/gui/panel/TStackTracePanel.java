...
public final class TStackTracePanel extends TRefreshablePanelElement
{
	...
	protected final @Override void init()
	{
		...
		//title-bar
		final var panel_title = new TPanelElement(0, 0, getWidth(), getTextRenderer().lineHeight + 14)
		{
		...
		addChild(panel_title);
		{
			...
			ico.setTexture(new UITexture(
					ResourceLocation.fromNamespaceAndPath(getModID(), "textures/gui/icons.png"),
			...
			lbl.setText(literal(title).withStyle(ChatFormatting.BOLD));
		...
		addChild(panel_content);
		{
			...
			panel_d.setTooltip(Tooltip.create(TCDCT.gui_panel_scrollTip()));
			panel_content.addChild(panel_d);
			
			final var pd_lines = Arrays.stream(description.split("\\r?\\n")).map(s -> literal(s)).toArray(Component[]::new);
			...
			panel_content.addChild(panel_a, false);
			{
				...
				btn_copyTrace.setTooltip(Tooltip.create(translatable("chat.copy")));
