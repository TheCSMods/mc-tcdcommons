...
public abstract class TClickableWidget extends TElement implements IEnableStateProviderSetter
{
	 ...
	 */
	public static final WidgetSprites BUTTON_TEXTURES = AccessorPressableWidget.getButtonTextures();
	
	/**
	 * {@link TCDCommons}'s widget textures image.
	 */
	public static final ResourceLocation T_WIDGETS_TEXTURE = ResourceLocation.fromNamespaceAndPath(getModID(), "textures/gui/widgets.png");
	// --------------------------------------------------
	...
	public @Override boolean input(final TInputContext inputContext)
	{
		...
		switch(inputContext.getInputType())
		{
			...
			case KEY_RELEASE:
				...
				if(!(keyCode == KEY_RETURN || keyCode == KEY_NUMPADENTER/* || keyCode == 32*/)) break;
				//click
				click(true);
				return true;
