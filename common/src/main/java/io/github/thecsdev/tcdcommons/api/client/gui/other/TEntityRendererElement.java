...
public @Virtual class TEntityRendererElement extends TBlankElement
{
	...
	protected @Nullable MultiLineLabel entityTypeName;
	...
	{
		final var textRenderer = MC_CLIENT.font;
		if(entityType != null)
			this.entityTypeName = MultiLineLabel.create(textRenderer, entityType.getDescription(), this.width);
		else this.entityTypeName = MultiLineLabel.create(textRenderer, TextUtils.literal("-"));
	}
	//
	...
	{
		this.entityScale = Mth.clamp(entityScale, 0.1, 5);
		recalcCache_mobSize();
	}
	// --------------------------------------------------
	/**
	...
	protected final void recalcCache_cXY()
	{
		...
		{
			final int fh = MC_CLIENT.font.lineHeight;
			this.entityTextY =
				(getY() + (getHeight() / 2)) +
				(fh / 2) -
				(this.entityTypeName.getLineCount() * fh);
		}
	...
	public @Virtual @Override void render(TDrawContext pencil)
	{
		...
		try
		{
			//throw new RuntimeException("Cannot render entities");
			pencil.drawTEntity(
					this.entity,
