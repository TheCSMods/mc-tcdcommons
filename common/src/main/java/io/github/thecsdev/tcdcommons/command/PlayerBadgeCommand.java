...
public final class PlayerBadgeCommand
{
	...
	public static void register(CommandDispatcher<CommandSourceStack> dispatcher)
	{
		...
		final var command = literal("badges").requires(scs -> config.enablePlayerBadges && scs.hasPermission(2));
	...
	private static ArgumentBuilder<CommandSourceStack, ?> badges_edit()
	{
		return literal("edit")
				.then(argument("targets", EntityArgument.players())
						.then(argument("badge", ResourceLocationArgument.id()).suggests(SUGGEST_PB)
								.then(literal("set")
	...
	// --------------------------------------------------
	private static SuggestionProvider<CommandSourceStack> SUGGEST_PB = (context, builder) ->
	{
	...
	private static int execute_edit(CommandContext<CommandSourceStack> context, boolean setOrIncrease)
	{
		try
		{
			...
			final var arg_badge = ResourceLocationArgument.getId(context, "badge");
			...
			//send feedback
			context.getSource().sendSuccess(() -> TCDCT.cmd_pb_edit_out(
	...
	private static int execute_clear(CommandContext<CommandSourceStack> context)
	{
		try
		{
			...
			//send feedback
			context.getSource().sendSuccess(() -> TCDCT.cmd_pb_clear_out(TextUtils.literal(Integer.toString(affected.get()))), false);
	...
	private static int execute_query(CommandContext<CommandSourceStack> context)
	{
		try
		{
			...
			//execute
			context.getSource().sendSuccess(() -> TCDCT.cmd_pb_query_out(
	...
	// ==================================================
	public static @Internal void handleError(CommandContext<CommandSourceStack> context, Throwable e)
	{
		//handle errors
